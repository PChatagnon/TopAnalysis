...
#include "TopLJets2015/TopAnalysis/interface/ExclusiveTop_NoTriggerCut.h"
...
int dump_index_1(vector<double> sum_squared){
    int index=0;
    double a=0;
    double b=0;
    a=sum_squared[0];
...
double bjet_err_1(double p,double eta,double phi, int err_type){
    eta=abs(eta);
...
double ljet_err_1(double p,double eta,double phi, int err_type){
    eta=abs(eta);
...
double mu_err_1(double p,double eta,double phi, int err_type){
    eta=abs(eta);
...
double e_err_1(double p,double eta,double phi, int err_type){
    eta=abs(eta);
...
//      MAIN
// ---------------------------------------------------------------------------------------------------------------------
void RunExclusiveTop_NoTrig(TString filename,
                     TString outname,
...
//    }
    ...
    TString fvars[]={
        ...
        "nBjets", "nLightJets", "nJets", "minEtaJets" ,"maxEtaJets", "ht", "cat","sumPFPz","sumPFEn","sumPVChPz","sumPVChEn","nchPV",
    ...
    ADDVAR(&(outVars["ch_tag"]),"ch_tag","/F",outPT);
    ...
    SelectionTool selector(filename, false, triggerList, SelectionTool::AnalysisType::NOTRIG);
	...
	std::vector<Jet>      all_jets;
        ...
        for(size_t ij=0; ij<allJets.size(); ij++) {
			...
			all_jets.push_back(allJets[ij]);
                        if(allJets[ij].pt()<30. || fabs(allJets[ij].eta())>2.4){continue;}//The Pt and Eta cut on jets is placed here to keep all jets for rapidity analysis
	...
	// Jet structure and rapidity gap
	// Look for the higher rapidity of the jets

	float min_eta = 5.0;	
	float max_eta = -5.0;
	for( size_t i_jets=0;i_jets<all_jets.size();i_jets++){

		float eta_jets = all_jets[i_jets].p4().Eta();
		if(eta_jets>max_eta)max_eta=eta_jets;
		if(eta_jets<min_eta)min_eta=eta_jets;
        }
	outVars["minEtaJets"]=min_eta;
	outVars["maxEtaJets"]=max_eta;

	//PF Candidates E and PZ
	float sumPFPz = 0.0;
	float sumPFEn = 0.0;
	auto  sumPVChPz = 0.0;
        auto sumPVChEn = 0.0;
	auto nchPV = 0;
	for (int ipf=0; ipf<8; ipf++) {
		sumPFPz+= ev.sumPFPz[ipf];
		sumPFEn+= ev.sumPFEn[ipf];
	}
	sumPVChPz= ev.sumPVChPz;
        sumPVChEn= ev.sumPVChHt;
	nchPV = ev.nchPV;
	outVars["sumPFPz"]=sumPFPz;
	outVars["sumPFEn"]=sumPFEn;
	outVars["sumPVChPz"]=sumPVChPz;
        outVars["sumPVChEn"]=sumPVChEn;
	outVars["nchPV"]=nchPV;

    // ---- EVENT SELECTION --------------------------------------------------------------
	//cout<<" Channel Tag "<<chTag<<" nb leptons "<< selectedLeptons.size()<<endl;
	...
	if (selectedLeptons.size()!=1) continue; // ONLY events with 1 selected lepton
        ...
        if(bJets.size()>=2 && lightJets.size()>=2)        {    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            //cout<<"b jet size "<<bJets.size()<<" light jet size "<<lightJets.size()<<endl;
            ...
            int correct_index=dump_index_1(mistake);   // index of best candidate
...
//            double m_nu      =   neutrino.M();
            ...
            double e_b1_eta=bjet_err_1(abs(pt_b1/sin(theta.Eval(eta_b1))),eta_b1,phi_b1,1);
            double e_b1_p=sqrt(pow(bjet_err_1(abs(pt_b1/sin(theta.Eval(eta_b1))),eta_b1,phi_b1,0)/(sin(theta.Eval(eta_b1))),2)+pow(pt_b1/pow(sin(theta.Eval(eta_b1)),2)*cos(theta.Eval(eta_b1))*err_theta_fracdeltaeta.Eval(eta_b1)*e_b1_eta,2));
            double e_b1_phi=bjet_err_1(abs(pt_b1/sin(theta.Eval(eta_b1))),eta_b1,phi_b1,2);
            double e_b1_theta=e_b1_eta*err_theta_fracdeltaeta.Eval(eta_b1);

            double e_b2_eta=bjet_err_1(abs(pt_b2/sin(theta.Eval(eta_b2))),eta_b2,phi_b2,1);
            double e_b2_p=sqrt(pow(bjet_err_1(abs(pt_b2/sin(theta.Eval(eta_b2))),eta_b2,phi_b2,0)/(sin(theta.Eval(eta_b2))),2)+pow(pt_b2/pow(sin(theta.Eval(eta_b2)),2)*cos(theta.Eval(eta_b2))*err_theta_fracdeltaeta.Eval(eta_b2)*e_b2_eta,2));
            double e_b2_phi=bjet_err_1(abs(pt_b2/sin(theta.Eval(eta_b2))),eta_b1,phi_b2,2);
            double e_b2_theta=e_b2_eta*err_theta_fracdeltaeta.Eval(eta_b2);

            double e_q1_eta=ljet_err_1(abs(pt_q1/sin(theta.Eval(eta_q1))),eta_q1,phi_q1,1);
            double e_q1_p=sqrt(pow(ljet_err_1(abs(pt_q1/sin(theta.Eval(eta_q1))),eta_q1,phi_q1,0)/(sin(theta.Eval(eta_q1))),2)+pow(pt_q1/pow(sin(theta.Eval(eta_q1)),2)*cos(theta.Eval(eta_q1))*err_theta_fracdeltaeta.Eval(eta_q1)*e_q1_eta,2));
            double e_q1_phi=ljet_err_1(abs(pt_q1/sin(theta.Eval(eta_q1))),eta_q1,phi_q1,2);
            double e_q1_theta=e_q1_eta*err_theta_fracdeltaeta.Eval(eta_q1);

            double e_q2_eta=ljet_err_1(abs(pt_q2/sin(theta.Eval(eta_q2))),eta_q2,phi_q2,1);
            double e_q2_p=sqrt(pow(ljet_err_1(abs(pt_q2/sin(theta.Eval(eta_q2))),eta_q2,phi_q2,0)/(sin(theta.Eval(eta_q2))),2)+pow(pt_q2/pow(sin(theta.Eval(eta_q2)),2)*cos(theta.Eval(eta_q2))*err_theta_fracdeltaeta.Eval(eta_q2)*e_q2_eta,2));
            double e_q2_phi=ljet_err_1(abs(pt_q2/sin(theta.Eval(eta_q2))),eta_q2,phi_q2,2);
            double e_q2_theta=e_q2_eta*err_theta_fracdeltaeta.Eval(eta_q2);

            double e_l_eta,e_l_p,e_l_phi,e_l_theta;
            if(m_l>=0.1){
              e_l_eta=mu_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,1);
              e_l_p=sqrt(pow(mu_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,0)/(sin(theta.Eval(eta_l))),2)+pow(pt_l/pow(sin(theta.Eval(eta_l)),2)*cos(theta.Eval(eta_l))*err_theta_fracdeltaeta.Eval(eta_l)*e_l_eta,2));
              e_l_phi=mu_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,2);
              e_l_theta=e_l_eta*err_theta_fracdeltaeta.Eval(eta_l);
            } else {
              e_l_eta=e_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,1);
              e_l_p=sqrt(pow(e_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,0)/(sin(theta.Eval(eta_l))),2)+pow(pt_l/pow(sin(theta.Eval(eta_l)),2)*cos(theta.Eval(eta_l))*err_theta_fracdeltaeta.Eval(eta_l)*e_l_eta,2));
              e_l_phi=e_err_1(abs(pt_l/sin(theta.Eval(eta_l))),eta_l,phi_l,2);
              e_l_theta=e_l_eta*err_theta_fracdeltaeta.Eval(eta_l);
            }
