...
class PPSEff
{
	...
	TString era[static_cast<int>(Periods::Count)] = {
		"UNDEF","2017B","2017C1","2017C2","2017D","2017E","2017F1","2017F2","2017F3","2017H"
	};
	...
	void init(std::string filename){
		...
		else if(filename.find("EraH_interpotEfficiency")!=std::string::npos){
			// load eraH multitrack eff
			hname ="Arm0_st2_rp3/h2InterPotEfficiencyMapMultiRP_arm0_st2_rp3";
			_heff201745[9]  = (TH2D *)_file0->Get(hname);
			if(!_heff201745[9]) {std::cout << "ERROR init() " << hname<< " not loaded " << std::endl; return;}				
			hname = "Arm1_st2_rp3/h2InterPotEfficiencyMapMultiRP_arm1_st2_rp3";
			_heff201756[9]  = (TH2D *)_file0->Get(hname);
			if(!_heff201756[9]) {std::cout << "ERROR init() " << hname<< " not loaded " << std::endl; return;}								
			_heff201756[9]->Draw("colz");
			_heff201745[0] = (TH2D *)_heff201745[9]->Clone("empty"); _heff201745[0]->Reset();
			_heff201756[0] = (TH2D *)_heff201756[9]->Clone("empty"); _heff201756[0]->Reset();
			EffInit=true;
			addUncFlat = 0;
		}
		else{
