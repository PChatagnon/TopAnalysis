...
int main(int argc, char* argv[]){
	...
	for(int h=0; h < nevents; h++){ 
		...
		double sumPVChPz = 0.0;//tree->GetLeaf("sumPVChPz")->GetValue(0);
		double sumPVChEn = 0.0;//tree->GetLeaf("sumPVChEn")->GetValue(0);
		...
		if ((bool(Xip1>0.0) ^ bool(Xip2>0.0)) && cat==5.){

			float xi_tt_plus = (1./(13000.))*( (pair).E()+(pair).Pz());
			float xi_tt_minus = (1./(13000.))*( (pair).E()-(pair).Pz());


			float xi_PF_plus = (1./(13000.))*( sumPVChEn+sumPVChEn);
			float xi_PF_minus = (1./(13000.))*( sumPVChEn-sumPVChEn);			
