...
int main(int argc, char *argv[])
{
    ...
    TH1F* ch_tag = (TH1F*)oldfile->Get("ch_tag");
    

    TH1F* nbjets = (TH1F*)oldfile->Get("nbjets");
    TH1F* njets = (TH1F*)oldfile->Get("njets");
    TH1F* nvtx_hist = (TH1F*)oldfile->Get("nvtx");
    ...
    PPSEff *MultiRP_eff = new PPSEff(Form("%s/EraH_interpotEfficiency_reMiniAOD.root", data_path.Data()));
    ...
    for (iMCEntry = 0; (iMCEntry < nEventsToMix) && (iMCEntry + nMCEventsToSkip < nMCEntries); iMCEntry++)
    {
        ...
        if (isSignal)
        {
            if (p1_xi > 0)
            {
                ...
                cout<<"Multi RP eff "<<(MultiRP_eff->getEff(p1_x, p1_y, 0, run))<<endl;
        ...
        if (mode_Double_PU && isSignal && p1_xi == 0 && p2_xi > 0)
        ...
        else if (mode_Double_PU && isSignal && p1_xi > 0 && p2_xi == 0)
	    ...
	    ///!!!!!!!!!!!!!! Multiply PPS weight by two to account for arm choice
            ptag_wgt = proton_config_probability[arm_PPS]*2.0;
            //ptag_wgt *= ptr.trueZeroTracksRatio(run, beamXangle, 0);
