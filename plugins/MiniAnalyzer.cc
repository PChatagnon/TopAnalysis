...
void MiniAnalyzer::genAnalysis(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //cout<<"Events is valid "<<evt.isValid()<<endl;
  if(evt.isValid())
    {
	  ...
	  for(int i=0; i< ev_.g_npsw; i++){ ev_.g_psw[i]=evt->weights().at(i);/*cout<<evt->weights().at(i)<<endl;*/}
	}
  ...
  //cout<<"Events is valid "<<evet.isValid()<< "  "<<evet->weights()[0].wgt<<" "<<ev_.g_npsw<<endl;
  if(evet.isValid()) 
    {
	   ...
	   for (unsigned int i=0; i<evet->weights().size(); i++) {
		   ...
		   if (id == 2000) ev_.g_w[6]=evet->weights()[i].wgt; // central value
		   if (id>=2001 && id <= 2100) ev_.g_w[id-1994]=evet->weights()[i].wgt; // PDF eig.
		   if (id == 2101) ev_.g_w[107]=evet->weights()[i].wgt; // aS=0.116
		   if (id == 2102) ev_.g_w[108]=evet->weights()[i].wgt; // aS=0.120
